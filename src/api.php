...
function item_to_api(array $r, bool $withImages = false): array
{
    ...
    $out = [
        ...
        // The packaging template this release was made from.
        //
        // A fact about the release, so it lives on the title - "big box, two
        // floppies and a manual" describes an edition rather than one copy of
        // it. Reported on the entry because that is where somebody is looking
        // when they wonder where the shape came from: the hardware side has
        // always named its model and the software side never did, which made
        // templates look like something only machines used.
        'software_model' => ($r['software_model_id'] ?? null) === null ? null : [
            'id'   => (int) $r['software_model_id'],
            'name' => (string) $r['software_model_name'],
            'slug' => (string) $r['software_model_slug'],
        ],
