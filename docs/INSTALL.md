...
database connection, loads the schema and structure data, writes
...
| `[instance]` | `name`, `tagline`, `url` (where the engine answers), `client_url` (where the web interface is - a separate application on its own address or path, so an engine at `https://retro.example` serving its client from `/web` wants `https://retro.example/web`; the same address as `url` is refused, since it would redirect to itself. Without it `/` answers 503 once the install finishes; the API and the phones work regardless), `currency` (shown prices; written to the configuration and to the Currency setting the General tab edits), `library_autofill` (whether a library somebody creates starts with the filing structure copied in - on by default, examples never), `timezone`, `trusted_proxies`, `debug`, `debug_status` |
...
```
vault read -field=answers secret/retrohive | php bin/install.php --answers - --quiet
```
...
CREATE USER 'retrohive'@'localhost' IDENTIFIED BY 'a-long-random-password';
GRANT ALL PRIVILEGES ON retrohive.* TO 'retrohive'@'localhost';
FLUSH PRIVILEGES;
```
...
```bash
mariadb -u retrohive -p retrohive < db/schema.sql
mariadb -u retrohive -p retrohive < db/seed.sql
```
...
```bash
sudo mkdir -p /opt/retrohive
sudo cp -a . /opt/retrohive-core/
cd /opt/retrohive
```
...
```bash
sudo chown -R root:root /opt/retrohive
sudo chown -R www-data:www-data /opt/retrohive-core/public/uploads
sudo chmod 775 /opt/retrohive-core/public/uploads
```

Nothing else needs to be writable. If SELinux is enforcing:

```bash
sudo semanage fcontext -a -t httpd_sys_rw_content_t "/opt/retrohive-core/public/uploads(/.*)?"
sudo restorecon -Rv /opt/retrohive-core/public/uploads
```
...
<VirtualHost *:80>
    ...
    CustomLog /var/log/apache2/retrohive-access.log combined
</VirtualHost>
```
...
server {
    ...
    root /opt/retrohive-core/public;
    index index.php;
...
The app works out its own prefix, so `https://host/retrohive-core/` needs no
changes. If a proxy rewrites paths in a way that confuses the detection, set it
explicitly:

```apache
SetEnv APP_BASE_PATH /retrohive
```
...
mariadb -u retrohive -p retrohive -e "
...
```bash
./bin/backup.sh /srv/backups/retrohive
```
...
```bash
gunzip -c db-20260725-033000.sql.gz | mariadb -u retrohive -p retrohive
tar -C /opt/retrohive-core/public -xzf uploads-20260725-033000.tar.gz
```
...
`sudo -u www-data touch /opt/retrohive-core/public/uploads/test`
...
account is gone:
`mariadb -u retrohive -p retrohive -e "UPDATE users SET role='admin' WHERE username='you';"`

**Swedish characters look wrong**
The database was not created with `utf8mb4`. Check with
`SHOW CREATE DATABASE retrohive;` and convert:

```sql
ALTER DATABASE retrohive CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci;
